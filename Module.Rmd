...
This is us pulling in all the data that we will need in this module
...
#We will use tree data from Harvard to start and then apply this methodology to our own data that we created. We will look at the distributions of three different tree species. 
HTrees <- read.csv("http://harvardforest.fas.harvard.edu/data/p03/hf032/hf032-01-tree.csv")
```


Before we get started, you should know the two different data types that GIS spatial statistics can analyze. Vectors and Rasters. Vectors are continous sets of data that contain polygos, lines, and/or point data. Rasters on the other hand are images made up of pixels, with each pixel having a single corresponding value. Spatial autocorrelation can be performed on both polygon data and raster data, but the majority of packages for spatial statistics require you to have a raster data set. 

For point data you can convert to either raster or a polygon. The point to polygon transformation can be particularly useful. Polygons help visualize nearest neighbors. One type of polygon is the priximity polygon, or Voronoi polygon, which has each polygon surrounding the area of an object or point.
