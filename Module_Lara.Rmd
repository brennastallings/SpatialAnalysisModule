...
<<<<<<GREG ADD EXAMPLE OF DIFFERENT DISTRIBUTIONS 
...
<<<<GREG ADD PHOTO/EXPLANATION? 

#Spatial Weight (GREG THERE ARE SOME THINGS LATER ON THAT CAN BE MOVE UP HERE)
...
-Rook, queen, bishop moves for raster data <<<<GREG EXAMPLE!!!!
-Distance bands (discrete neighborhoods define by distance from a cell or polygon) <<<<EXAMPLE
-Nearest K neighbor 
...
<<<<<<GREG DEFINE MONTE CARLO VS NORMAL METHODS
...
```
>>>>GREG 1 sentence interpretation 
...
# Generate neighbors list 
nb.unif <- cell2nb(nrow = nrows(unif.rast), ncol = ncols(unif.rast))
# To calculate neighbors for queen configuration
...
moran.acru.mc
```
>>>>GREG EXPLALIN MORANS I INTERPREATION AND THE GRAPH
>>>>BRENNA I LIKE THE CORRELOGRAMS BUT WE SHOULD ADD IN THE ACTUAL STATISTICS TEST AS WELL FOR MORAN.TEST AND MORAN.MC. SAME WITH LISA AND GEARY C TESTS. 
...
```{r}
#Moran's I
random.moran <- correlog(coordinates(unif.spdf), d$att, method = "Moran", nbclass = NULL, alternative = "two.sided")
...
```{r}
#Moran's I
unif.moran <- correlog(unif.spdf, v$att, method = "Moran", nbclass = NULL, alternative = "two.sided")
# Can view the textual results by simply typing 'random.moran' 
plot(unif.moran) #plots the results
moran.out.unif <- moran.test(unif.df$V3, nb2listw(nb.unif, style = "B")) 
moran.out.unif
moran.unif.mc<- moran.mc(unif.df$V3, nb2listw(nb.unif, style = "B"), nsim=99)
moran.unif.mc
...
#Geary's C
unif.geary <- correlog(coordinates(unif.spdf), v$att, method = "Geary", alternative = "two.sided")
plot(unif.geary)
```
>>>>>BRENNA THIS AINT WORKING 
...
>>>>>>WE NEED TO ADD LOCAL G IN HERE TO 
...
<<<<<GREG PUT SPATIAL WEIGHTS UP TOP
...
g
```
>>>>BRENNA GGPLOT DID NOT WORK.
***fixed
>>>>>>>ALSO CAN WE PUT THIS LAST AFTER THE STATS. 
...
<https://cran.r-project.org/web/packages/lctools/vignettes/SpatialAutocorrelation.pdf> 
<https://mltconsecol.github.io/TU_LandscapeAnalysis_Documents/Assignments_web/Assignment06_Autocorrelation.pdf> Harvard Trees
<https://gdmdata.com/media/documents/handouts/2017ASA_FarmToTable_Spatial_correlation.pdf> 
<https://rstudio-pubs-static.s3.amazonaws.com/223305_944ddc517306448f8fb0d60ca29dd94b.html> Examples for greg?


=======
>>>>>>> dae30cae85a084964d5061e14f2ddfa13dc21e99
