...
# of aggregation among like categories; add the argument 'alternative='less'
# to reverse this)
joincount.test(as.factor(random.rast@data@values), lwb, alternative = "greater")
# Second, the permutation-based jon count test; similar to above, and you
# can adjust the number of simulations with the 'nsim' argument
joincount.mc(as.factor(random.rast@data@values), lwb, nsim = 999, alternative = "greater")
# Can also compute these for the queen setup; for example, with the
# permutation test:
joincount.mc(as.factor(random.rast@data@values), lwb.queen, nsim = 999, alternative = "greater")
```
<<<<BRENNA WHY IS THERE A PLOT OF POINTS??? 
*this is gone now
...
Moran's I
```{r}
moran.out.acru<-moran.test(acru$dbh91, nb2listw(nb.acru, style = "B")) 
